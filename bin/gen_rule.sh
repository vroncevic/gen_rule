...
function __gen_rule() {
	...
	if [[ -n "${OP}" && -n "${TD}" ]]; then
		...
		__info_debug_message "$MSG" "$FUNC" "$GEN_RULE_TOOL"
		__progress_bar PB_STRUCTURE
		declare -A config_gen_rule=()
		__load_conf "$GEN_RULE_CFG" config_gen_rule
		STATUS_CONF=$?
		declare -A config_gen_rule_util=()
		__load_util_conf "$GEN_RULE_UTIL_CFG" config_gen_rule_util
		STATUS_CONF_UTIL=$?
		...
		if [ $STATUS -eq $NOT_SUCCESS ]; then
			MSG="Force exit!"
			__info_debug_message_end "$MSG" "$FUNC" "$GEN_RULE_TOOL"
			exit 129
		fi
		...
		if [ $STATUS -eq $SUCCESS ]; then
			...
			__info_debug_message_end "Done" "$FUNC" "$GEN_RULE_TOOL"
			exit 0
		fi
		__usage GEN_RULE_USAGE
		exit 130
	fi
	__usage GEN_RULE_USAGE
	exit 128
